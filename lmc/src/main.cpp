...
int main(int argc, char *argv[]) {
	...
	//	Force the Fusion style everywhere. Native styles (especially macOS's)
	//	silently ignore parts of our stylesheet -- e.g. QToolButton's
	//	down-arrow subcontrol image, or the item view's own accessibility
	//	focus ring drawn outside Qt's paint system -- which is exactly what
	//	caused the black focus outline and mismatched icons the person
	//	reported. Fusion is a pure Qt style, so our QSS renders identically
	//	on every platform.
...
//#endif
	...
	//	Apply the INYTEL brand stylesheet application-wide
	QFile styleFile(":/style/inytel");
	...
	//	Note: this app's own encryption (crypto.cpp) links OpenSSL directly and
	//	does not depend on Qt's QSslSocket/TLS backend, so we no longer warn
	//	the user about it -- it was a false alarm on platforms where Qt's TLS
	//	backend isn't found (e.g. Homebrew Qt on macOS) even though nothing
	//	in the app actually needs it (the update-check URL is plain http).
