...
void lmcMainWindow::init(User* pLocalUser, QList<Group>* pGroupList, bool connected) {
    ...
    ui.tvUserList->setRootIsDecorated(false);
#ifdef Q_OS_MAC
    ui.tvUserList->setAttribute(Qt::WA_MacShowFocusRect, false);
#endif
	...
	index = qMax(index, 0);
	QPixmap statusPix(statusPic[index], "PNG");
	btnStatus->setIcon(QIcon(statusPix));
...
#ifdef Q_OS_MAC
	//	On macOS, Qt promotes this action into the native app menu, which
	//	renders its own monochrome icon style -- our colored icon looks out
	//	of place there, so we leave it unset and let macOS handle it.
	settingsAction = pToolsMenu->addAction("&Preferences",
...
#ifdef Q_OS_MAC
	aboutAction = pHelpMenu->addAction("&About", this, SLOT(trayAboutAction_triggered()));
#else
