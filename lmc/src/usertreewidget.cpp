...
void lmcUserTreeWidgetDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
	...
	if(type == "Group") {
		//	fill with a fixed white (matches our own list background,
		//	resources/inytel.qss) instead of palette.window(), which turns
		//	dark in macOS/Windows dark mode and was the real cause of the
		//	"black border" -- this margin was always the system window
		//	color showing through around the smaller rounded pill.
		painter->fillRect(itemRect, QColor("#ffffff"));

		//	INYTEL style: soft indigo pill instead of the harsh system highlight color
		QColor fillColor("#e0e7ff");   // indigo-100
		QColor textColor("#4f46e5");   // indigo-600
		...
		//	fixed brand colors instead of QPalette::Base/AlternateBase/Shadow,
		//	which all turn dark in dark mode (and Shadow is always a
		//	near-black color) -- same root cause as the Group branch fix
		//	above, just for individual contact rows.
		bool selected = option.state.testFlag(QStyle::State_Active) &&
			option.state.testFlag(QStyle::State_Enabled) &&
			option.state.testFlag(QStyle::State_Selected);

		QColor fillColor;
		if(selected)
			fillColor = QColor("#e0e7ff");			// indigo-100
		else if(index.row() % 2 == 1)
			fillColor = QColor("#f8fafc");			// slate-50
		else
...
void lmcUserTreeWidgetDelegate::drawCheckBox(QPainter *painter, const QPalette& palette,
	...
	QRectF box(checkBoxRect.left() + 1, checkBoxRect.top() + 3, 15, 15);
	...
	if(checkState == Qt::Checked) {
		...
		QPointF p1(box.left() + 3.2, box.top() + 7.8);
		QPointF p2(box.left() + 6.2, box.top() + 10.8);
		QPointF p3(box.left() + 11.5, box.top() + 4.2);
